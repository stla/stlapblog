...
Distributions of Beta family generate random values in the interval $(0,1)$, but from a technical perspective they are more difficult to deal with than the normal (Gaussian) distribution. 
...
Thus, Bem's method, up to approximation, falls into the method I called [*Reducing a model to get a confidence interval*][stla2]. Thus this method should approximately control the significance level as long as the approximation is not bad. Moreover, from the previously seen closeness between the $t$-test interval and the PL interval, I suspect that the method is close to optimatility when it is valid. 
...
<div id="wrap">
<iframe id="frame" src="assets/htmlframes/hbinom_logit02.html"></iframe>
</div>
