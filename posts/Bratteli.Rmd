...
library(diagram) 
Bgraph <- function(fun_Mn, N, title=NA, blackpath=NULL, col_path="blue", labels_path=FALSE, labels_vertex=TRUE, USE.COLNAMES=FALSE, first_vertex=0, label_root="ø", only_end=FALSE, cex_vertex=1.5, labels_edges=TRUE, cex_edge=1.1, ellipse_vertex=FALSE, ellipse_edge=FALSE, LaTeX=FALSE, ...){
  ...
  for(k in 2:N){
    v <- vpath[k-1] 
    vpath[k] <- which(Mn[[k]][v,]>0)[blackpath[k]]
  }
  blackpath <- vpath 
  ...
  for (i in 1:nr){ # draw and store the arrows
    ...
    # labels on edges 
    if(labels_edges && !labels_path){ # faire un black path only
...
Bgraph(Pascal_Mn, N=3, blackpath=c(1,0,1), first_vertex=1)
```
...
```{r}
par(mar=c(.1,.1,.1,.1))
Bgraph(Pascal_Mn, N=3, blackpath=c(1,0,1), first_vertex=1, hor=FALSE)
```
...
```{r}
par(mar=c(.1,.1,.1,.1))
Bgraph(Pascal_Mn, N=3, blackpath=c(1,0,1), first_vertex=0, labels_path=TRUE, only_end=TRUE)
```
...
par(mar=c(.1,.1,.1,.1))
Bgraph(Pascal_Mn, N=4, blackpath=c(1,0,1,1), labels_path=TRUE, USE.COLNAMES=TRUE, ellipse_vertex=TRUE)
```
...
Bgraph(fun_Mn, N=3, labels_vertex=TRUE, blackpath=c(2,1,2), labels_path=TRUE)
```
...
Bgraph(fun_Mn, N=3, labels_vertex=FALSE, labels_edges=FALSE, blackpath=c(2,1,2))
```
...
par(mar=c(.1,.1,.1,.1))
Bgraph(Pascal_Mn, N=3, blackpath=c(1,0,1), labels_path=TRUE, USE.COLNAMES=TRUE, LaTeX=TRUE, label_root="$\\varnothing$", relsize=.6)
dev.off()
# convert to pdf: 
