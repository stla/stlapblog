...
L'espace des qubits purs est de dimension $2$, et il peut être représenté par une sphère, appelée [*sphère de Bloch*](https://en.wikipedia.org/wiki/Bloch_sphere), mais qui n'est rien d'autre que la [*sphère de Riemann*](https://fr.wikipedia.org/wiki/Sph%C3%A8re_de_Riemann). Les angles $\theta$ et $\phi$ correspondent aux coordonnées sphériques polaires de $\ket{\psi}$ sur cette représentation (respectivement la latitude et la longitude) :

<img src="./assets/img/BlochSphere_01.gif" alt="BlochSphere01" width="55%" class="center">

En inteprétant naïvement cette représentation, on aurait évidemment tort de penser que $\ket{1}=-\ket{0}$, comme le suggère cette représentation euclienne.   De même, l'expression $(\star)$ du qubit pur $\ket{\psi}$ comme une combinaison linéaire des deux états de base $\ket{0}$ et $\ket{1}$ ne signifie pas que cette même combinaison linéaire a lieu dans l'espace tridimensionnel.

L'isomorphisme entre l'espace des qubits purs et la sphère de Bloch se justifie de la façon suivante. Elle repose sur le vecteur $\xi$ dessiné en rouge sur la figure, qui est la [projection stéréographique](https://fr.wikipedia.org/wiki/Projection_st%C3%A9r%C3%A9ographique) de $\ket{\psi}$ de (la représentation de) $\ket{\psi}$. Notez que sur ma figure, pour des raisons esthétiques, la projection stéréographique part du pôle Sud, alors qu'on la fait plus habituellement partir du pôle Nord. 

L'espace des qubits purs précédemment définis comme des classes d'équivalence est l'[espaces des rayons projectifs](https://fr.wikipedia.org/wiki/Espace_projectif_de_Hilbert) de l'espace de Hilbert $\mathbb{C}^2$, aussi appelé la [droite projective complexe](https://fr.wikipedia.org/wiki/Droite_projective), et noté  $\mathbb{C}\mathbb{P}^1$ ou $\mathbb{P}^1(\mathbb{C})$. Il est bien connu que la droite projective complexe est isomorphe l'espace $\bar{\mathbb{C}}$ des nombres complexes augmenté d'*"un point à l'infini"* (le compactifié d'Alexandroff de $\mathbb{C}$). Cet isomorphisme envoie le qubit pur $\ket{\psi}$ sur 
$$
