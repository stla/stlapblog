...
```{r, eval=FALSE}
Fmu(mean(x)) # 0.5 ?
q <- seq(mean(x)-3*sd(x)/sqrt(n), mean(x)+3*sd(x)/sqrt(n), length.out = 10)
points(q, sapply(q, function(t) sum(W*(Mu<t))))
# sims
msample <- rmultinom(1, length(Mu), W)[,1]
resims <- rep(Mu, times=msample)
curve(ecdf(resims)(x), add=TRUE, col="blue")
```
