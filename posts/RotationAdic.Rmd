...
Notant $\xi_0$ la partition mesurable de $(0,1)$ en singleton, la paire de partitions mesurable $(\xi_0, \xi_1)$ s'identifie alors à la paire de tribus $({\cal F}_0, {\cal F}_1)$ définie par ${\cal F}_1=\sigma(U_1)$ et ${\cal F}_0=\sigma(U_1, U_0)$ où :
...
Mais chaque autre point vert dans  $\xi'_2(x')$ correspond lui aussi à un représentant-$\xi_1$ point sur l'arc rouge par $g_1^{-1}$, et ainsi à une classe-$\xi_1$ sur l'espace $(0,1)$ de départ (le grand cercle). 

*(à faire: une figure avec les autres orbites bleues)*

La classe-$\xi_2$ de $x$ dans la partition mesurable $\xi_2 \prec \xi_1$ est alors la réunion des $\#\xi'_2\bigl(\xi_1(x)\bigr)$ classes-$\xi_1$ correspondant à chaque point vert.  
...
Il y a deux graphes possibles à associer à la construction, selon que l'on utilise ou non des arêtes multiples.

### Graphe avec arêtes multiples 
